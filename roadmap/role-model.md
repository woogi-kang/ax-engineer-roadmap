...
**AX Engineer**는 조직의 업무 문제를 찾고 업무 흐름을 다시 설계하며, AI를 기존 데이터·시스템·권한 구조에 연결하는 엔지니어다. 구현한 기능을 실제 업무에 적용하고 다른 사람이 이어서 운영할 수 있는 상태까지 책임진다.
...
- 평가·보안·관측성·복구
- 현업 정착과 운영 인수인계
- 다른 업무에서도 재사용할 규칙과 도구의 검증

현재 공개된 역할 설명을 검토한 내용은 [AX Engineer 공개 역할 검토](../research/ax-engineer-role-review.md)에서 확인할 수 있다.

## 2. 책임의 끝

AX Engineer의 책임은 프로토타입이 동작할 때 끝나지 않는다. 업무 결과와 운영 책임이 연결되고 다른 사람이 안전하게 이어받을 수 있을 때 한 업무의 전환이 마무리된다.

```mermaid
flowchart LR
    D["Discover<br/>업무·사용자·제약 파악"] --> R["Redesign<br/>업무 흐름 재설계"]
    R --> B["Build<br/>작은 범위를 끝까지 구현"]
    B --> O["Operate<br/>배포·평가·복구"]
    O --> A["Adopt<br/>현업 정착·인수인계"]
    A --> S["Standardize<br/>재사용 규칙과 공통 도구"]
    S -->|"운영에서 얻은 학습"| D
```

AX Engineer가 모든 결정을 혼자 내리는 것은 아니다. 업무 책임자, 데이터 소유자, 보안·법무 담당자, 기존 시스템 운영자와 함께 판단한다. AX Engineer는 함께 내린 결정을 구현하고 운영 기록으로 남기는 책임을 맡는다.
...
초기에는 한 사람이 세 모델의 일을 모두 맡을 수 있다. 사례가 늘어나면 업무 발견·구현·운영·지원이 한 사람에게 몰려 병목이 되지 않도록 책임을 나눠야 한다.

이 표는 AX Engineer를 조직 안에 어디에 배치할지 설명한다. 운영 모델과 개인이 맡는 업무 전환·기반·확산 책임의 비중은 별개의 문제다. 한 운영 모델 안에서도 여러 책임 조합이 가능하다.
...
- 데이터 원본과 업무 용어를 연결하고, 접근 권한과 시스템 경계를 설계한다.
- 모델·검색·규칙·도구 호출을 업무에 필요한 수준으로만 선택한다.
- 평가, 실행 기록, 승인, 관측성, 비용 한도, 복구를 구현한다.
- 반복되는 실행 규칙과 구성 요소를 공통 하네스로 만든다.
...
한 직무가 세 축을 모두 깊게 맡아야 하는 것은 아니다. 다만 한 업무를 운영 환경에 배포하려면 각 책임을 누가 맡는지 설명할 수 있어야 한다.
...
| AI 업무 시스템 구현 | AX·엔지니어링 책임자 | 직접 또는 공동으로 구현하고 운영 기록 제공 |
| 현업 정착과 절차 변경 | 조직·업무 책임자 | 검수·교육·지원·이중 운영·종료 조건 설계 |
...
- 한 번 구현한 기능을 공통 기반으로 발전시킬 절차가 있는가?
...
- 운영 시스템을 만들라고 요구하면서 데이터·권한·배포 환경에는 접근할 수 없다.
- 보안과 승인은 다른 팀이 알아서 처리한다고 가정한다.
- 새 방식이 현업에 자리 잡지 않아도 기능 출시만으로 완료라고 본다.
- 공통 플랫폼을 요구하지만 두 번째 업무에서 실제로 재사용한 결과가 없다.
