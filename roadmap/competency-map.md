...
AX Engineer의 역량은 여덟 영역으로 나눈다. 모든 영역을 같은 깊이로 다룰 필요는 없지만, 한 업무를 운영 환경에 배포하려면 각 영역의 책임자가 누구인지 설명할 수 있어야 한다.
...
- 특정 도구를 전제로 하지 않고 성공과 중단 조건을 설명한다.
...
- 기준선과 지표 정의
- 업무 후보 평가표
- 범위와 이번 범위에서 제외한 목표가 포함된 결정 기록

### 실패 대응

다음 신호가 보이면 문제 정의와 투자 결정을 다시 검토한다.
...
다음 상태가 보이면 목표 흐름과 책임 배분을 다시 검토한다.
...
- 한 업무의 원본 데이터·가공 데이터·분석 결과·실행 결과를 분리한다.
- 데이터 계약과 용어집을 작성한다.
...
다음 상태가 보이면 데이터 사용과 원본 연결을 중단하고 바로잡는다.
...
- 두 개 이상의 기존 시스템을 연결해 작은 범위의 기능을 끝까지 구현한다.
- 인증, 권한, 실패 처리, 중복 방지, 감사 로그를 포함한다.
- 다른 개발자가 로컬 또는 테스트 환경에서 같은 결과를 재현할 수 있게 한다.
...
다음 상태를 시험에서 발견하고 배포 전에 중단·복구할 수 있어야 한다.
...
- 규칙 기반 처리·검색·분류·생성·에이전트 중 필요한 수준을 선택한다.
...
- 위험한 결과가 나오면 실행을 중단하거나 담당자에게 넘기는 규칙을 구현한다.
...
다음 상태가 나오면 평가 범위와 실행 권한을 다시 제한한다.

- 좋은 예시 몇 개를 본 뒤 품질을 확정
- LLM 평가 점수를 담당자 검토 없이 절대 기준으로 사용
...
- 자동 재시도, 즉시 중단, 담당자에게 넘기기 중 하나를 실패 유형별로 선택한다.
...
- 운영자가 문제 원인과 영향을 같은 식별자로 추적할 수 있게 한다.
...
- 구현자의 설명 없이도 운영자가 핵심 흐름을 수행할 수 있는지 확인한다.
- 사용자 피드백을 기능 요청과 업무 문제로 분리한다.
- 교육·지원·인수인계 절차를 운영하고 기존 절차의 종료 조건을 확인한다.
...
다음 상태가 이어지면 교육뿐 아니라 절차와 책임 구조를 다시 살핀다.

- 로그인 수나 생성 건수만 보고 현업 정착을 판단
- 사용률이 낮은 이유를 현업의 변화 의지 부족으로 단정
...
- 운영 실패를 재현 가능한 형태로 정리해 공통 기반의 개선 과제로 만든다.

### 적용

- 두 번째 업무에서 기존 실행 규칙과 구성 요소를 재사용한다.
...
다음 상태가 보이면 공통화 범위를 줄이고 업무별 차이를 다시 기록한다.
...
각 영역에서 가장 높은 단계에 맞추지 말고, **다른 사람이 확인할 결과물과 기록이 있는 단계**까지만 현재 수준으로 표시한다.

| 역량 | 이해 | 선택 | 적용 | 검증 | 실패 대응 | 다음 결과물 |
|---|---:|---:|---:|---:|---:|---|
| 문제 발견과 사업 판단 |  |  |  |  |  |  |
| 업무·조직 설계 |  |  |  |  |  |  |
| 데이터와 의미 |  |  |  |  |  |  |
