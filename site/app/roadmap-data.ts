...
export const roadmapGroups: RoadmapGroup[] = [
  {
    ...
    nodes: [
      node(
        ...
        "역할 선택",
        "Persona selection",
        "업무 정의·구현·투자·통제 중 자신이 맡은 책임을 확인한다.",
      ...
      node(
        ...
        "업무·데이터·시스템·권한·운영·현업 정착의 준비 상태로 첫 실행 범위를 정한다.",
  ...
  },
  {
    ...
    prompt: {
      ko: "AX 엔지니어의 책임과 현업·리더·데이터·보안·운영 담당자의 협업 책임을 구분한다.",
    ...
    nodes: [
      ...
      node(
        ...
        "현재·목표 업무, 검수, 예외, SOP, 현업 정착과 기존 절차 종료 기준을 정한다.",
      ...
      node(
        ...
        "AX 엔지니어 실행 경로",
        "AX Engineer path",
        "소프트웨어·AI·통합·평가·배포·복구를 실제로 운영할 수 있는 업무 시스템으로 연결한다.",
  ...
  },
  {
    ...
    nodes: [
      ...
      node(
        ...
        "에이전트 시스템",
        "Agent systems",
        "Agent Loop가 필요한 조건과 상태·중단·메모리·담당자 인계 기준을 정한다.",
  ...
  },
  {
    ...
    prompt: {
      ko: "순서대로 진행하되 운영 결과와 새로운 제약이 발견되면 앞 단계로 돌아간다.",
      en: "Proceed in order, returning to earlier stages when operating results or new constraints require it.",
    },
    nodes: [
      node("stage-1", "목표와 경계", "Goals and boundaries", "무엇을 바꾸고 AI가 하지 않을 일을 정한다.", "Define what changes and what AI must not do.", "기준선·비목표·중단 조건", "Baseline, non-goals, and stop conditions", "delivery-lifecycle/01-outcomes-and-boundaries.md"),
  ...
  },
  {
    ...
    nodes: [
      ...
      node("project-2", "담당자 승인 워크플로우", "Human-approved workflow", "담당자가 근거와 변경 전후를 보고 승인·수정·거절한다.", "Approve, edit, or reject after reviewing evidence and before-and-after state.", "승인 기준·우회 시험·수정 부담", "Approval rubric, bypass tests, and correction burden", "projects/02-human-approved-workflow.md"),
      ...
      node("project-5", "두 번째 업무 재사용", "Second-workflow reuse", "실제로 재사용한 실행 규칙과 업무별 추가 항목을 구분한다.", "Separate execution rules actually reused from workflow-specific additions.", "업무 비교·추가 비용·공통화 결정", "Workflow comparison, addition cost, and standardization decision", "projects/05-second-workflow-reuse.md", ["builder", "leader", "guardian"]),
    ],
  },
  {
    id: "scale",
