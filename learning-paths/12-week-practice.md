...
12주 동안 하나의 업무를 발견하고 구현한 뒤, 두 번째 업무에서 무엇을 재사용할 수 있는지 확인한다. 조직 실전 트랙은 배포·운영·현업 정착까지 진행하고, 공개 시뮬레이션 트랙은 격리된 환경에서 배포와 복구를 연습한다. 매주 만든 기능뿐 아니라 판단 근거와 확인 결과도 함께 남긴다.

```mermaid
flowchart LR
    C1["사이클 1 · 1~4주<br/>문제와 해법 검증"] --> C2["사이클 2 · 5~8주<br/>운영 배포와 현업 정착"]
...
- 외부 시스템을 변경하는 실행은 담당자 승인과 복구 경로가 준비된 뒤 추가한다.
- 매주 `계속 / 수정 / 보류 / 중단` 중 하나를 근거와 함께 결정한다.
- 도구와 모델을 바꾸더라도 업무 식별자, 평가, 승인, 기록은 이어지게 만든다.
- [역할 선택 안내](../start-here/persona-selector.md)에서 자신이 맡을 AX 책임과 협업할 역할을 확인한다.
- [조직 준비도](../start-here/organization-readiness.md)에서 실제 실행이 가능한 범위를 확인한다.

## 실습 트랙 선택

조직 시스템과 실제 사용자에게 접근할 수 있는지에 따라 시작 전에 트랙을 고른다. 두 트랙이 비슷한 결과물을 만들더라도 실제로 확인할 수 있는 범위는 다르다.
...
| 확인 가능한 것 | 운영 배포, 현업 사용, 인수인계, 절차 변경 | 설계, 구현, 평가, 복구 연습, 사용성 피드백 |
| 주장할 수 없는 것 | 측정하지 않은 조직 성과 | 실제 현업 정착, 공식 절차 변경, 조직 성과 |

공개 시뮬레이션도 기술과 판단 능력을 보여 주는 결과물이 될 수 있다. 다만 실제 조직에서 배포·운영한 결과는 아니므로 사례 제목과 결과에 `시뮬레이션`임을 표시한다.

다섯 [실습 프로젝트](../projects/README.md)는 이 12주 경로의 중간 점검으로 사용한다. 1~4주에는 안전한 보조 도구, 5~6주에는 담당자 승인과 샌드박스 통합, 7~8주에는 운영 파일럿 또는 시뮬레이션, 9~12주에는 두 번째 업무의 재사용 결과를 만든다.

## 사이클 1. 문제와 해법 검증

### 1주차: 업무 선택과 기준선

- 사용자, 업무 책임자, 오류나 오작동의 영향을 받을 사람을 구분한다.
- 업무 후보 세 개를 가치·빈도·근거·데이터·위험·복구 가능성으로 비교한다.
- 선택한 업무의 현재 흐름, 병목, 대기, 예외를 기록한다.
- 성공·중단 조건과 초기에 제외할 목표를 정한다.
...
### 2주차: 업무 흐름 재설계와 데이터 계약

- 현재 단계를 제거·통합·표준화·AI 보조·담당자 승인·자동 실행으로 다시 분류한다.
- 최신 상태를 확인할 원본 시스템과 데이터 소유자를 정한다.
...
- 다른 개발자가 테스트 환경에서 핵심 흐름을 재현할 수 있는지 확인한다.
...
- 조직 실전 트랙은 실제 사용 현황을, 공개 시뮬레이션은 독립 검토자가 발견한 사용성 문제를 기록한다.
...
- 조직 실전 트랙은 구현자가 아닌 운영자가 대표 흐름과 예외를 직접 처리할 수 있는지 확인한다.
- 공개 시뮬레이션은 독립 검토자가 설명서만 보고 같은 흐름을 재현할 수 있는지 확인한다.
- 수정 부담, 반복 사용, 지원 요청, 업무 결과 또는 사용성 문제를 확인한다.
- 조직 실전 트랙은 교육·지원·문의 경로와 운영 책임을 인계하고 기존 절차의 상태를 결정한다.
- 공개 시뮬레이션은 실제 현업 정착과 절차 변경을 주장하지 않고 검토 한계를 기록한다.

결과물:

- 사용자 검수 또는 독립 검토 기록
- 교육·지원 문서
- 현업 사용·업무 결과 검토 또는 시뮬레이션 한계
- 기존 절차 종료·유지 결정 또는 가상 전환 조건

## 사이클 3. 재사용과 조직 확장

### 9주차: 두 번째 업무 선택과 실행 규칙

- 첫 업무와 비슷하지만 사용자·데이터·위험 중 하나가 다른 업무를 고른다.
- 기존 실행 규칙과 구성 요소를 그대로 재사용, 설정 변경, 새 구현으로 나눈다.
...
- 첫 업무의 실행 규칙·코드·템플릿을 사용해 두 번째 업무의 작은 기능을 끝까지 구현한다.
...
- 두 업무에서 확인한 입력·출력·평가·승인·기록·복구 규칙만 공통 하네스에 포함한다.
- 버전·호환성·업무별 추가 항목·폐기 정책과 팀 자율성의 경계를 정한다.
- 조직 실전 트랙은 중앙 AX팀·현업·IT·데이터·보안의 책임과 현재 성숙도를 정리한다.
- 공개 시뮬레이션은 목표 조직과 권한에 대한 가정을 실제 사실과 구분한다.
- 문제, 선택, 구현, 실패, 운영, 현업 정착, 재사용을 하나의 사례로 정리한다.
...
- 구현자가 아닌 사람이 핵심 흐름과 대표 예외를 직접 확인했다.
- 두 번째 업무에서 재사용한 부분과 실패한 추상화를 설명할 수 있다.
- 조직 공통 기준과 팀 자율성의 경계를 문서화했다.
- 성과와 한계를 같은 사례에 함께 기록했다.

조직 실전 트랙의 추가 완료 조건:

- 실제 운영 범위, 실제 사용과 현업 정착, 인수인계, 기존 절차의 상태를 확인했다.

공개 시뮬레이션 트랙의 추가 완료 조건:

- 샌드박스·테스트 환경과 독립 검토 범위를 공개했다.
- 실제 현업 정착, 공식 절차 변경, 조직 성과를 주장하지 않았다.
