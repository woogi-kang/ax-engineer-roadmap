...
이 모델은 조직이 AI 도구를 얼마나 많이 쓰는지가 아니라, AI가 참여하는 핵심 업무를 얼마나 안전하게 추가하고 운영하며 개선할 수 있는지를 본다.
...
[조직 준비도 진단](../start-here/organization-readiness.md)은 특정 업무를 지금 어떤 방식으로 시작할 수 있는지 본다. 이 문서의 성숙도는 같은 운영 기준과 역량을 여러 업무에 반복 적용할 수 있는지 본다. 준비도가 높은 한 업무가 있다고 조직 전체의 성숙도가 높은 것은 아니다.
...
- 팀마다 다른 기술을 쓰더라도 공통 작업 규칙을 공유한다.
...
M4에서도 위험한 업무는 담당자가 판단하고 승인하도록 유지할 수 있다. 자동 실행 범위를 넓히는 것보다 위험에 맞는 통제를 선택하는 것이 중요하다.

## 3. 진단 차원

| 차원 | M0 | M1 | M2 | M3 | M4 |
|---|---|---|---|---|---|
| 업무 포트폴리오 | 개인 선택 | 후보와 실험 기준 | 운영 중인 업무 검토 | 조직 우선순위 | 지속적 투자·중단 |
| 데이터 | 개인 파일 | 업무별 출처 | 계약·소유자·계보 | 공통 의미·접근 기반 | 품질 피드백을 데이터 계약과 개선 계획에 반영 |
| 실행 | 복사·붙여넣기 | 담당자 실행 | 승인·제한 실행 | 공통 실행 정책 | 현업이 안전하게 확장 |
| 평가 | 주관적 확인 | 업무별 사례 | 회귀·운영 평가 | 공통 평가 기준 | 실패 결과를 표준 개선에 반영 |
| 통제 | 개인 판단 | 금지선 | 권한·감사·복구 | 중앙 정책과 팀 경계 | 위험 기반 지속 조정 |
| 현업 정착 | 자발적 사용 | 일부 사용자 참여 | 공식 절차와 인수인계 | 교육·지원·자립 | 역할과 절차 지속 개선 |
| 재사용 | 없음 | 템플릿 후보 | 업무 내 재사용 | 여러 팀의 공통 기반 | 운영 결과를 공통 기반에 반영 |
...
조직 전체에 영구적인 완료 상태는 없다. 다만 다음 조건을 충족하면 **1차 운영 체계를 갖췄다**고 판단할 수 있다.

- 핵심 업무 포트폴리오와 우선순위가 있다.
- 운영 업무마다 데이터 조건, 실행 규칙, 평가 기준이 있다.
- 사람과 AI의 권한·승인·책임이 분명하다.
- 실행·변경·승인·복구 이력을 추적할 수 있다.
- 모델 품질, 업무 결과, 실제 사용, 비용, 안정성을 함께 검토한다.
- 현업이 공통 기반 위에서 새 업무를 제안하고 운영할 수 있다.
- 중앙 AX팀이 모든 일상 운영의 병목이 아니다.
- 기존 절차와 공통 자산을 종료할 기준도 있다.

1차 완료는 새 업무나 규제가 생겨도 같은 방식으로 준비 상태를 확인하고, 작은 범위에서 검증한 뒤, 실제 운영 결과를 바탕으로 확대하거나 중단할 수 있는 상태를 뜻한다.
