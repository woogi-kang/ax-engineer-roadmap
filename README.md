...
AX 엔지니어가 바꿀 업무를 고르고, AI를 기존 데이터와 시스템에 연결하고, 실제 운영까지 이어 가는 데 필요한 로드맵이다. 한국 조직에서 자주 마주치는 결재·권한·규제·디지털 격차도 함께 다룬다.
...
4. [실습 프로젝트](projects/README.md): 안전한 보조 도구부터 두 번째 업무의 재사용 검증까지 진행한다.
5. [운영과 조직 확장](organization-maturity/README.md): 여러 업무에서 같은 운영 능력을 반복할 수 있는지 확인한다.

[인터랙티브 로드맵 열기](https://ax-engineer-roadmap-korea.woogi-dev.chatgpt.site/) · [사이트 실행 안내](site/README.md)

## AX 업무 전환 8단계

8단계는 순서대로 진행하되, 운영 결과나 새로운 제약이 발견되면 앞 단계로 돌아가 다시 확인한다.
...
1. **알기** — 개념과 원리를 자신의 말로 설명할 수 있는가?
2. **판단하기** — 조건과 위험을 비교해 이 방법을 선택한 이유를 설명할 수 있는가?
3. **해보기** — 실제 업무와 비슷한 데이터·권한·예외 조건에서 직접 수행했는가?
4. **결과 남기기** — 다른 사람이 검토하거나 재현할 수 있는 결과물과 기록을 남겼는가?
5. **실패 다루기** — 자주 발생하는 오판과 실패를 발견하고, 필요할 때 중단하거나 복구할 수 있는가?
...
[비개발자를 위한 AX 용어 안내](start-here/non-developer-glossary.md)에서 LLM, RAG, Agent, MCP, 평가, 로그, 수동 대체를 실제 업무 장면에 맞춰 설명한다.
